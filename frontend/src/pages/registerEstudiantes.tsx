...
const RegisterEstudiantes: React.FC = () => {
  ...
  const handleSendCode = async () => {
    ...
    setOtpSending(true);
    try {
      const { error: otpError } = await sendEmailOtp(trimmedEmail, { shouldCreateUser: true });
      if (otpError) {
  ...
  const handleResendCode = async () => {
    ...
    setAlert(null);
    try {
      const { error: resendError } = await sendEmailOtp(trimmedEmail, { shouldCreateUser: true });
      if (resendError) {
  ...
  const verifyCodeWithSupabase = async (joined: string) => {
    ...
    try {
      const { error: verifyError } = await verifyEmailOtp({ email: trimmedEmail, token: joined });
      if (verifyError) {
        setEmailVerified(false);
