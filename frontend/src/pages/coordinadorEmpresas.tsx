...
const EmpresasPage = () => {
  ...
  const availableFilters = useMemo(() => {
    const createOptions = (key: keyof Empresa) => {
      ...
      const sortedValues = Array.from(values).sort((a, b) => collator.compare(a, b));
  ...
  const sortedEmpresas = useMemo(() => {
    ...
    return sorted;
  }, [collator, filteredEmpresas, sortOption]);
  ...
  const handleFilterChange = useCallback(
    (field: keyof EmpresaFilters) => (event: SelectChangeEvent<FilterValue>) => {
