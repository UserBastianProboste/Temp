...
const WIDGETS: WidgetDefinition[] = [
  ...
  { id: 'approvals-queue', title: 'Prácticas por aprobar' },
...
const WIDGET_SPANS: Partial<Record<WidgetId, { xs?: number; md?: number; xl?: number }>> = {
  'approvals-queue': { md: 2, xl: 2 },
  'company-health': { xl: 2 },
};

const resolveSpan = (id: WidgetId, breakpoint: 'xs' | 'md' | 'xl') => {
  const config = WIDGET_SPANS[id];
  if (!config) return 1;
  if (config[breakpoint]) {
    return config[breakpoint] as number;
  }

  if (breakpoint === 'xl') {
    return config.md ?? config.xs ?? 1;
  }
...
export default function CoordinatorDashboard() {
      ...
      <Stack spacing={2}>
        ...
        <Grid container spacing={2}>
          {entries.map(([estado, value]) => (
            <Grid item xs={12} sm={6} key={estado}>
              <Box
                sx={{
                  borderRadius: 2,
                  p: 2,
  ...
  const renderReportsTracker = () => {
    ...
    if (insights.error) {
      return <Alert severity="warning">{insights.error}</Alert>;
    }

    if (insights.loading && reportsSummary.total === 0) {
      return <LinearProgress />;
    }
        ...
        {orderedWidgets.map((id) => {
          ...
          return (
            <Box
              ...
              sx={{
                gridColumn: {
                  xs: `span ${resolveSpan(id, 'xs')}`,
                  md: `span ${resolveSpan(id, 'md')}`,
                  xl: `span ${resolveSpan(id, 'xl')}`,
                },
                minWidth: 0,
              }}
            >
