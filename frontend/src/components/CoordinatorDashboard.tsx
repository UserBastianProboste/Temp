...
const WIDGETS: WidgetDefinition[] = [
  { id: 'pending-students', title: 'Perfiles pendientes' },
  { id: 'practice-pipeline', title: 'Estado de prácticas' },
  { id: 'approvals-queue', title: 'Prácticas por aprobar', span: { lg: 2, xl: 2 } },
  { id: 'upcoming-calendar', title: 'Próximos hitos' },
  { id: 'reports-tracker', title: 'Seguimiento de informes' },
  { id: 'company-health', title: 'Salud de empresas colaboradoras' },
  { id: 'activity-feed', title: 'Últimas novedades' },
];

const buildGridColumnSpan = (widget: WidgetDefinition) => {
  const span = widget.span ?? {};
  const resolve = (key: Breakpoint, fallback: number): number => span[key] ?? fallback;
  const xs = resolve('xs', 1);
  const sm = resolve('sm', span.xs ?? xs);
  const md = resolve('md', span.sm ?? span.xs ?? xs);
  const lg = resolve('lg', span.md ?? span.sm ?? span.xs ?? xs);
  const xl = resolve('xl', span.lg ?? span.md ?? span.sm ?? span.xs ?? xs);
    ...
    <Box sx={{ position: 'relative', pb: 8 }}>
      ...
      >
        {orderedWidgets.map((id) => {
          ...
          const columnSpan = buildGridColumnSpan(widget);

          return (
            <Box key={id} sx={{ gridColumn: columnSpan, minWidth: 0 }}>
              <Card
                variant="outlined"
                sx={{
                  height: '100%',
