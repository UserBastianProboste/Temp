      ...
      try {
        ...
        const [studentsResponse, informesResponse] = await Promise.all([
          ...
          supabase
            .from('informes')
            .select(
              'id, estudiante_id, nombre, ruta, created_at, nota, tipo, tipo_practica, bucket',
            )
        ...
        if (informesResponse.error) {
          ...
          const acceptedBuckets = ['informe_avance_practica', 'informe_final_practica'];
          const informesFiltrados = (informesResponse.data ?? []).filter((inf: InformeRecord) => {
            if (!inf) return false;

            const bucket = (inf.bucket ?? '').toString().toLowerCase();
            if (bucket && acceptedBuckets.includes(bucket)) {
              return true;
            }

            const textFields = [inf.nombre, inf.tipo, inf.tipo_practica, inf.ruta]
              .map((value) => normalize(value))
              .join(' ');

            if (textFields.includes('informe') && textFields.includes('practica')) {
              return true;
            }
