    ...
    const load = async () => {
      try {
        ...
        if (informesResponse.error) {
          ...
          const rawInformes = (informesResponse.data ?? []).filter(Boolean) as InformeRecord[];
          const informesFiltrados = rawInformes.filter((inf: InformeRecord | null) => {
            if (!inf) return false;
            ...
            const containsPractica = [bucket, nombre, tipo, tipoPractica].some((value) =>
              value.includes('practica'),
            );
            const containsInforme = bucket.includes('informe') || nombre.includes('informe');

            return containsPractica || containsInforme;
          });
          setReports((informesFiltrados.length > 0 ? informesFiltrados : rawInformes) as InformeRecord[]);
        }
      } catch (err: any) {
        if (mounted) {
