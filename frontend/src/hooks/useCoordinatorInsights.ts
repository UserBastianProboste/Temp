...
const PRACTICE_BUCKET_KEYWORDS = ['informe_avance_practica', 'informe_final_practica'];

const isPracticeReport = (inf: InformeRecord) => {
  const bucket = inf.bucket?.toLowerCase() ?? '';
  const nombre = inf.nombre?.toLowerCase() ?? '';
  const tipo = inf.tipo?.toLowerCase() ?? '';
  const tipoPractica = inf.tipo_practica?.toLowerCase() ?? '';

  if (bucket && PRACTICE_BUCKET_KEYWORDS.some((keyword) => bucket.includes(keyword))) {
    return true;
  }

  if (nombre && PRACTICE_BUCKET_KEYWORDS.some((keyword) => nombre.includes(keyword))) {
    return true;
  }

  if (tipoPractica) return true;
    ...
    const load = async () => {
      try {
        ...
        if (informesResponse.error) {
          ...
          const informesData = (informesResponse.data ?? []) as InformeRecord[];
          const filtrados = informesData.filter(isPracticeReport);
          setReports(filtrados.length > 0 ? filtrados : informesData);
        }
      } catch (err: any) {
        if (mounted) {
