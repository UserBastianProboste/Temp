...
const resolveEstadoLabel = (estado: string): PracticeRecord['estado'] => {
  ...
  if (normalized.includes('progreso')) return 'En progreso';
  if (normalized.includes('por aprobar') || normalized.includes('poraprobar')) return 'Pendiente';
  if (normalized.includes('revision')) return 'Pendiente';
...
export function usePracticasDashboard() {
  ...
  const pipeline = useMemo(() => {
    return records.reduce(
      (acc, item) => {
        const key = resolveEstadoLabel(item.estado);
        acc[key] = (acc[key] ?? 0) + 1;
        return acc;
      },
      {
  ...
  const pendingQueue = useMemo(() => {
    return records
      .filter((item) => resolveEstadoLabel(item.estado) === 'Pendiente')
      .sort(
